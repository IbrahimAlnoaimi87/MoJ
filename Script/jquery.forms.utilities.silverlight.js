(function($) {
	...
	function silverlightBox(options) {
		...
		function open() {
			...
			if ($window.length === 0) {
				$window = $('<div></div>').attr({ id: options.window.id }).css(options.window.css);
				$("body").append($window)
				;
			}
			var $overlay = $('#' + options.overlay.id);
			if ($overlay.length === 0) {
				$overlay = $('<div></div>').attr({ id: options.overlay.id })
					.css(options.overlay.css)
					.on("click", function(e) { close(); return false; })
					;
				$("body").append($overlay);
			}
			...
			$window.silverlight(options.silverlight);
			resize();
			$(document).on('keydown.silverlightbox', function(e) {
				if (e.keyCode === 27/*ESC*/) {
					close();
				}
				return true
			});
			$(window).on('resize.silverlightbox', function() {
				resize();
			});
		...
		function close() {
			$(window).off('resize.silverlightbox');
			$(document).off('keydown.silverlightbox');
